...
reMarked = function(opts) {
  ...
  //  comp_style: false,      // use getComputedStyle instead of hardcoded tag list to discern block/inline
    unsup_tags: {       // handling of unsupported tags, defined in terms of desired output style. if not listed, output = outerHTML
      ...
      inline: "sup sub i u b center big",
      // eg: "\n<tag>\n\tsome content\n</tag>"
    //  block1: "",
      // eg: "\n\n<tag>\n\tsome content\n</tag>"
    ...
    initK: function()
    {
      ...
      if (this.e.hasChildNodes()) {
        ...
        if (nodeName(this.e) == "table") { 
          if (this.e.hasChildNodes() && !this.e.tHead && this.e.tBodies[0].childNodes[0].childNodes.length > 1) {
            //if need to add header - single cell tables do not need one
        ...
        if (wrapInCode && !cfg.unsup_tags.dontUnwrap.test(nodeName(this.e))){
          this.e = document.createElement("code");
          this.e.innerHTML = codeStr;
          this.lnInd = 4;
        }
        ...
        for (i in this.e.childNodes) {
          ...
          if (name != "code" && nodeName(node.e) == "code"){
            node.wrap = ["\n",""];
            node.tagr = false;
          }
  ...
  lib.blk = lib.tag.extend({
    wrap: ["\n\n", function(){return this.brForSiblings()}],
    wrapK: null,
    tagr: false,
    lnInd: null,
    init: function(e, p ,i) {
    ...
    brForSiblings: function(){
      var siblings = this.p.c;
      // return "";
      if (siblings[siblings.length-1]==this || nodeName(this.p.e) == "a")
        return "";
      else
        return "\n\n";
    },
  ...
  lib.tblk = lib.blk.extend({tagr: false});
